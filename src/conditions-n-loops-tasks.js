 ...
 *  26  => XXVI
 */
function convertToRomanNumerals(inputNumber) {
  let finalNumber = inputNumber;
  let result = '';
 ...
 *  'qwerty', 'p'     => -1
 */
function getIndexOf(inputString, inputLetter) {
 ...
 *  12345, 6    => false
 */
function isContainNumber(inputNumber, inputDigit) {
  const inputString = String(inputNumber);

  for (let i = 0; i < inputString.length; i += 1) {
    if (inputString[i] === String(inputDigit)) {
      return true;
    }
  }
 ...
 */
function getBalanceIndex(inputArray) {
  let totalSum = 0;
  let leftSum = 0;
...
function sortByAsc(inputArray) {
  ...
  for (let i = 1; i < inputArray.length; i += 1) {
    if (inputArray[i] < current) {
      leftPart[leftIndex] = inputArray[i];
      leftIndex += 1;
    } else {
      rightPart[rightIndex] = inputArray[i];
      rightIndex += 1;
    }
  }
  ...
  for (let i = 0; i < rightIndex; i += 1) {
    result[leftIndex + 1 + i] = rightPart[i];
  }
  return result;
}
...
function getNearestBigger(number) {
  ...
  for (let l = i + 1; l < digits.length; l += 1) {
    rightArr[rightArr.length] = digits[l];
  }
